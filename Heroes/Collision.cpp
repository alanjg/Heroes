...
namespace XNA
{
	...
	// Transform a sphere by an angle preserving transform.
	//-----------------------------------------------------------------------------
	VOID TransformSphere(Sphere* pOut, const Sphere* pIn, XMMATRIX* Transform)
	{
		assert(pOut);
		assert(pIn);
		...
		// Load the center of the sphere.
		XMVECTOR Center = XMLoadFloat3(&pIn->Center);
		Center.m128_f32[3] = 1.0;
		XMVECTOR PointOnSphere = Center;
		PointOnSphere.m128_f32[0] += pIn->Radius;

		// Transform the center of the sphere.
		XMVECTOR TransformedCenter = XMVector3TransformCoord(Center, *Transform);
		XMVECTOR TransformedPoint = XMVector3TransformCoord(PointOnSphere, *Transform);
		
		// Store the center sphere.
		XMStoreFloat3(&pOut->Center, TransformedCenter);
		XMVECTOR dist = TransformedCenter - TransformedPoint;
		XMVECTOR radiusV = XMVector3Length(dist);
		// Scale the radius of the pshere.
		XMStoreFloat(&pOut->Radius, radiusV);
	...
	bool IntersectRayPlane(CXMVECTOR Origin, CXMVECTOR Direction, CXMVECTOR PlanePoint, CXMVECTOR PlaneNormal, XMVECTOR& intersectPoint, FLOAT* pDist)
	{
		XMVECTOR distV = XMVector3Dot(PlanePoint, PlaneNormal);
		float dist = -distV.m128_f32[0];

		XMVECTOR denV = XMVector3Dot(Direction, PlaneNormal);
		float den = -denV.m128_f32[0];
		if (den == 0)
		{
			return false;
		}
		XMVECTOR ndotoV = XMVector3Dot(PlaneNormal, Origin);
		float t = (dist + ndotoV.m128_f32[0]) / den;
		if (t <= 0.0f)
		{
			return false;
		}
		*pDist = t;
		...
		XMVECTOR tDir = XMVectorMultiply(Direction, tV);
		intersectPoint = XMVectorAdd(Origin, tDir);
		return true;
	}
